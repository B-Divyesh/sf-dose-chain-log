...
The static deployment target is `dose-chain-log.sociobot.in`; deploy exactly
the committed `dist/` output with:

```sh
/opt/fleet/lib/deploy-static.sh dose-chain-log dist
```

After deployment, run `/opt/fleet/lib/verify-url.sh` against the live URL and
the live axe CLI check. The factory can later build the Android debug APK in an
Android SDK/JDK worker with `cd android && ./gradlew assembleDebug`; device
testing remains necessary for notification delivery under battery restrictions.
